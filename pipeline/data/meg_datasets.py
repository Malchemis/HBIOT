...
class OnlineWindowDataset(torch.utils.data.Dataset):
    ...
    def _detect_system_ram_bytes() -> int:
        """Detect available RAM in bytes, respecting SLURM allocations.

        Checks SLURM_MEM_PER_NODE first (common on HPC clusters), then
        falls back to /proc/meminfo and os.sysconf.
        """
        import os

        # SLURM: use job allocation, not full node RAM
        slurm_mem = os.environ.get('SLURM_MEM_PER_NODE')
        if slurm_mem:
            try:
                return int(slurm_mem) * (1024 ** 2)  # MB -> bytes
            except ValueError:
                pass
        ...
        if slurm_mem_cpu and slurm_cpus:
            try:
                return int(slurm_mem_cpu) * int(slurm_cpus) * (1024 ** 2)
            except ValueError:
                pass
    ...
    def __getitem__(
        ...
        if self.is_test:
            ...
            # Total window count (same arithmetic as _build_index_map and create_windows)
            n_samples = self.cache.n_samples_list[rec_idx]
            total_windows = max(0, (n_samples - window_duration_samples) // window_step + 1)

            # Calculate window indices for this chunk
            start_window_idx = local_chunk_idx * num_context_windows
            end_window_idx = min(start_window_idx + num_context_windows, total_windows)

            # Slice only the needed windows directly from meg_data
            windows = np.array([
                meg_data[:, w * window_step : w * window_step + window_duration_samples]
                for w in range(start_window_idx, end_window_idx)
            ])
